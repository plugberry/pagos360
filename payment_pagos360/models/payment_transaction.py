...
class PaymentTransaction(models.Model):
    ...
    def _search_by_reference(self, provider_code, payment_data):
        ...
        """
        # Skip core's search for pagos360: it uses the same domain as the "else" branch below,
        # so it never finds anything the broader search here wouldn't, and it logs a warning
        # on the expected "not found yet" path (e.g. invoice barcode creates the tx afterwards).
        if provider_code != "pagos360":
        ...
        if not tx:
            # Info, not warning: expected when another module still has to create the tx (e.g. invoice barcode).
            _logger.info("Pagos360: No transaction found matching reference %s.", payment_data.get("ref"))
        return tx
