...
class PaymentTransaction(models.Model):
    ...
    def _pagos360_get_provider_invoice_from_reference(self, reference):
        ...
        if len(reference) == 3 and reference[0] == "inv":
            payment_provider_id = (
                self.env["payment.provider"].browse(int(reference[1])).exists()
            )
            account_move_id = (
                self.env["account.move"].browse(int(reference[2])).exists()
            )
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        """
        external_reference = notification_data.get("payload", {}).get(
            "external_reference"
        )
        ...
        if (
            ...
            and all(
                provider_invoice := self._pagos360_get_provider_invoice_from_reference(
                    external_reference
                )
            )
        ):
            ...
            if not tx:
                payment_method_id = (
                    ...
                    ._get_compatible_payment_methods(
                        provider.ids, invoice.partner_id.id
                    )
